...
void ClearPlayers::SetPos(VECTOR pos)
{
	for (auto& tran_ : transArray_)
	{
		tran_.pos = pos;
...
void ClearPlayers::Init3DModel(void)
{
	//役職モデルの設定
	for (int i = 0; i < PLAYERS; i++) {
...
void ClearPlayers::ResetAnim(const ANIM _anim, const float _speed)
{
